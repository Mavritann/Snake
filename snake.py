...
class MyWidget(Widget):
    def __init__(self, **kwargs):
        ...
        with self.canvas.after:
            ...
            Color(rgba = [random.random(), random.random(), random.random(), 1], group = "food") # группа для отдельного изменения цвета
            ...
            self.output = Label(pos = (250, 500), text = "", font_size = 50, color = (0.7, 0.28, 0.28, 1))
    ...
    def key_press(self, keyboard, keycode, text, modifiers):
        if keycode[1] == "up":
            Clock.schedule_interval(self.up, 1/3)
        elif keycode[1] == "down":
            Clock.schedule_interval(self.down, 1/3)
        elif keycode[1] == "left":
            Clock.schedule_interval(self.left, 1/3)
        elif keycode[1] == "right":
            Clock.schedule_interval(self.right, 1/3)
    ...
    def draw_food(self):
        # добавить проверку, чтобы не появлялось на змейке
        self.canvas.after.get_group("food")[0].rgba = [random.random(), random.random(), random.random(), 1]
        x = self.food_size * (random.randint(0, int(Window.width))//self.food_size)
        y = self.food_size * (random.randint(0, int(Window.height))//self.food_size)
    ...
    def new_element(self):
        ...
        if len(self.all_snake) == 1:
            if self.last_direction == "right":
                ...
                new.pos = (self.all_snake[-1].pos[0], self.all_snake[-1].pos[1] + self.food_size)       
        elif self.all_snake[-1].pos[0] - self.all_snake[-2].pos[0] < 0 and self.all_snake[-1].pos[1] == self.all_snake[-2].pos[1]:
            new.pos = (self.all_snake[-1].pos[0] - self.food_size, self.all_snake[-1].pos[1])
        elif self.all_snake[-1].pos[0] - self.all_snake[-2].pos[0] > 0 and self.all_snake[-1].pos[1] == self.all_snake[-2].pos[1]:
            new.pos = (self.all_snake[-1].pos[0] + self.food_size, self.all_snake[-1].pos[1])
        elif self.all_snake[-1].pos[1] - self.all_snake[-2].pos[1] < 0 and self.all_snake[-1].pos[0] == self.all_snake[-2].pos[0]:
            new.pos = (self.all_snake[-1].pos[0], self.all_snake[-1].pos[1] - self.food_size)
        elif self.all_snake[-1].pos[1] - self.all_snake[-2].pos[1] > 0 and self.all_snake[-1].pos[0] == self.all_snake[-2].pos[0]:
            new.pos = (self.all_snake[-1].pos[0], self.all_snake[-1].pos[1] + self.food_size) 

        self.all_snake.append(new)
        self.snake_positions.append(new.pos)
